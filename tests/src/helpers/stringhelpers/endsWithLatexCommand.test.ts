...
describe(endsWithLatexCommand.name, () => {
  it('returns true for latex commands with a slash and letters', () => {
    for (const s of [String.raw`\pi`, String.raw`2\pi`, String.raw`2\times\pi`, String.raw`\sin`]) {
      assert.isTrue(endsWithLatexCommand(s));
    }
  });
  it('returns false for other stuff', () => {
    for (const s of [String.raw`\pi^2`, String.raw`\sin6`, String.raw`\sin a`, '', String.raw`\|`, String.raw`\\`]) {
